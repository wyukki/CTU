...
int solve_problem(int **table, int *fin_array, int *counters,
    ...
    for (int i = 0; i < eggs_count; ++i) {
        ...
        if (!counters[step]) {
            if (number_of_empty_rows <= number_of_positions && !no_changes) {
                int last_number = table[number_of_positions - 1][index_in_table];
                for (int j = 0; j < number_of_positions - 1; ++j) {
                    if (last_number != table[j][index_in_table]) {
                        no_changes = false;
                        break;
                    } else {
                        no_changes = true;
            ...
            if (egg_column > 1 && step + 2 >= egg_column - 1) {
                table[egg_column - 2][next_column]++;
            }
            if (egg_column <= number_of_positions && step + 1 >= egg_column - 1) {
