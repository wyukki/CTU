        ...
        if (!counters[step]) {
            ...
            for (int j = 0; j < number_of_positions; ++j) {
                ...
                if (j == 0) {
                    ...
                    if (egg_column >= j + 1 && egg_column <= j + 2) {
                        table[j][next_column]++;
                        changed_at = j;
                    } else {
                        table[j][next_column] = table[j][index_in_table];
                    ...
                    if (egg_column >= number_of_positions) {
                        table[j][next_column]++;
                        changed_at = number_of_positions - 1;
                    } else {
                        table[j][next_column] = table[j][index_in_table];
                    }
                }
                if (changed_at >= 0) {
                    printf("change_at: %d\n", changed_at);
                    int value_of_change = table[changed_at][next_column];
                    printf("value_of_change = %d\n", value_of_change);
                    if (changed_at == 0) {
                        int poss_chage = table[changed_at + 1][next_column];
                        table[changed_at + 1][next_column] = value_of_change >= poss_chage
                                                                 ? value_of_change
                                                                 : poss_chage;
                    } else if (changed_at == number_of_positions - 1) {
                        int poss_change = table[changed_at - 1][next_column];
                        table[changed_at - 1][next_column] = value_of_change >= poss_change
                                                                 ? value_of_change
                                                                 : poss_change;
                    } else {
                        int left_poss_change = table[changed_at - 1][next_column];
                        int right_poss_change = table[changed_at + 1][next_column];
                        table[changed_at - 1][next_column] = value_of_change >= left_poss_change
                                                                 ? value_of_change
                                                                 : left_poss_change;
                        table[changed_at + 1][next_column] = value_of_change >= right_poss_change
                                                                 ? value_of_change
                                                                 : right_poss_change;
                    }
                    changed_at = -1;
