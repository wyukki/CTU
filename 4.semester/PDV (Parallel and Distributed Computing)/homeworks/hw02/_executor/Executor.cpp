...
long Executor::executeMethod(
    ...
    try {
        ...
        if (result == solution) {
            ...
            return static_cast<long>(chrono::duration_cast<chrono::nanoseconds>(end - begin).count());
        }
    } catch (...) {
        //ignored
    }
...
unsigned long Executor::findLengthOfShortestVector(const vector<vector<int8_t>> &data) const {
    ...
    const auto size = static_cast<unsigned long>(data.size());
...
#pragma omp parallel for if(size > static_cast<unsigned>(omp_get_max_threads()) * 1000)
    for (long i = 0; i < static_cast<long>(size); i++) {
        if (data[i].size() < minNumber) {
			# pragma omp critical
			{
				if (data[i].size() < minNumber) {
					minNumber = static_cast<unsigned long>(data[i].size());
				}
			}
        }
