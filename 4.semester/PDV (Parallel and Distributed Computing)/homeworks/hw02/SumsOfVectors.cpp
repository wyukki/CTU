...
void sumsOfVectors_omp_per_vector(const vector<vector<int8_t>> &data, vector<long> &solution,
                                  unsigned long minVectorSize) {
    unsigned long data_len = data.size();
    for (unsigned long i = 0; i < data_len; i++) {
        long sum = 0;
        unsigned long data_i_len = data[i].size();
        unsigned long j;
#pragma omp parallel for default(none) shared(data_i_len, data, i) reduction(+:sum)
        for (j = 0; j < data_i_len; ++j) {
            sum += data[i][j];
        }
        solution[i] = sum;
    }
...
void sumsOfVectors_omp_static(const vector<vector<int8_t>> &data, vector<long> &solution,
    ...
    long sum = 0;
#pragma omp parallel for default(none) reduction(+:sum) shared(data_len, data, solution) schedule(static)
    for (unsigned int i = 0; i < data_len; ++i) {
        sum = 0;
        unsigned long data_i_size = data[i].size();
        for (unsigned long j = 0; j < data_i_size; ++j) {
            sum += data[i][j];
        }
        solution[i] = sum;
    }
...
void sumsOfVectors_omp_dynamic(const vector<vector<int8_t>> &data, vector<long> &solution,
    ...
    unsigned int data_len = data.size();
    long sum = 0;
#pragma omp parallel for default(none) reduction(+:sum) shared(data_len, data, solution) schedule(dynamic)
    for (unsigned int i = 0; i < data_len; ++i) {
        sum = 0;
        unsigned long data_i_size = data[i].size();
        for (unsigned long j = 0; j < data_i_size; ++j) {
...
void sumsOfVectors_omp_shuffle(const vector<vector<int8_t>> &data, vector<long> &solution,
    ...
    unsigned int data_len = data.size();
    long sum = 0;
    std::vector<int> indexes(data_len);
    std::iota(indexes.begin(), indexes.end(), 0);
    std::shuffle(indexes.begin(), indexes.end(), std::mt19937(std::random_device()()));
#pragma omp parallel for default(none) reduction(+:sum) shared(data_len, data, solution, indexes) schedule(static)
    for (unsigned int i = 0; i < data_len; ++i) {
        sum = 0;
        int index = indexes[i];
        unsigned long data_i_size = data[index].size();
        for (unsigned long j = 0; j < data_i_size; ++j) {
            sum += data[index][j];
        }
        solution[index] = sum;
