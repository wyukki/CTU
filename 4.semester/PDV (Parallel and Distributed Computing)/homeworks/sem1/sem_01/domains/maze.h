...
public:
    ...
    std::vector<std::shared_ptr<const state>> next_states() const override {
		...
		const unsigned int cost_ns = UNIFORM ? 1 : (id % 5);
        if (!((*MAZE)[ (conf[0] - 1)*SIZE + conf[1]])){
            tmp_conf[0] = conf[0] - 1;
            succ.emplace_back(std::make_shared<maze_state<SIZE, UNIFORM>>(this->shared_from_this(), current_cost() + cost_ns, tmp_conf, MAZE, GOAL));
            tmp_conf[0] = conf[0];
        }
