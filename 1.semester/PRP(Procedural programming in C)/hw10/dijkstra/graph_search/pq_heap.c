...
_Bool pq_update(void *_pq, int label, int cost)
{
   ...
   pq_heap_s *pq = (pq_heap_s *)_pq;
   if (
       pq && pq->len < pq->size && label >= 0 && label < pq->size && pq->heapIDX[label] != -1)
   {
      int index = pq->heapIDX[label];
      int parent_idx = GET_PARENT(index);
      pq->cost[index] = cost;
      while (!ret && index > 0)
      {
         if (pq->cost[index] > pq->cost[parent_idx])
         {
            ret = true;
            break;
         }
         pq_swap(pq, index, parent_idx);
         index = parent_idx;
